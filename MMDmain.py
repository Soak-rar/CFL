...
def train(global_model_state_dict, dataset_dict, worker_id, device, epoch, delay, args):
    ...
    model_global.load_state_dict(global_model_state_dict)

    optimizer = optim.SGD(model.parameters(), lr=args.lr)
    mmd_optimizer = optim.SGD(model.fc2.parameters(), lr=args.lr)
    ...
    for local_epoch in range(args.local_epochs):
        for batch_index, (batch_data, batch_label) in enumerate(train_loader):
            ...
            mmd_loss = MMDLoss.data_mmd(model_global.fc2.state_dict()['weight'], model.fc2.state_dict()['weight'])
...
def save(global_test_eval, model_dict, args):
    args.to_string('MMDLoss  方法： 将mmd_loss 的权重设置为100%\n将参与训练的客户端设为40%')
