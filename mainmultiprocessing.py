...
def train(local_global_model_state_dict, local_shared_data, local_worker_id, device, local_epoch, q, local_args):
    model = init_model(local_args.dataset_name, local_args.data_classes)
    model.load_state_dict(local_global_model_state_dict)

    optimizer = optim.SGD(model.parameters(), lr=local_args.lr)
    dataset_dict = local_shared_data[local_worker_id]
    train_loader, test_loader = init_local_dataloader(dataset_dict, local_args)
    model.train()
    model.to(device)
    for local_epoch in range(local_args.local_epochs):
    ...
    model.to('cpu')
    q.put({'model_dict': model.state_dict(),
...
def init_shared_data(args):
    train_workers = Data.load_data(args)
    train_workers_dict = {i: train_workers[i] for i in range(args.worker_num)}
    shared_data_dict = Manager().dict(train_workers_dict)
...
def main(args):
    ...
    queue = Manager().Queue()
    shared_data = init_shared_data(args)
    ...
    for epoch in range(args.global_round):
        ...
        for worker_id in train_workers_id:
            pool.apply(func=train, args=(global_model.state_dict().copy(), shared_data, worker_id, device, epoch, queue, args))
            # train(global_model.state_dict().copy(), shared_data, worker_id, device, epoch, queue, args)

        while not queue.empty():
            train_eval = queue.get()
